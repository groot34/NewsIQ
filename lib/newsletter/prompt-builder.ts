...
export function buildArticleSummaries(articles: ArticleForPrompt[]): string {
  ...
  // Process articles (most recent first is assumed, but we'll take top N that fit)
  for (let i = 0; i < articles.length; i++) {
    const article = articles[i];
    
    // Truncate individual summary if too long
    let summaryContent =
      article.summary || article.content?.substring(0, MAX_CHARS_PER_ARTICLE) || "No summary available";
      
    if (summaryContent.length > MAX_CHARS_PER_ARTICLE) {
      summaryContent = summaryContent.substring(0, MAX_CHARS_PER_ARTICLE) + "...";
    }
...
`;
    ...
    currentTotalChars += formattedArticle.length;
    summaries.push(formattedArticle);
  }
